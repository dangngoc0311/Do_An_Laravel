...
$(function() {
    ...
    //////---------------13. Accordion -------------///////
    function panelAcc() {
        jQuery('#accordion h4[data-toggle="collapse"]').on(
            "click",
            function() {
                jQuery('#accordion h4[data-toggle="collapse"]').removeClass(
                    "collapsed"
                );
...
$(".btn-number").on("click", function(e) {
    ...
    var currentVal = parseFloat(input.val(), 10);
    if (!isNaN(currentVal)) {
        if (type == "minus") {
            if (currentVal > input.attr("min")) {
                input.val(currentVal - 0.1, 10).change();
            }
            if (parseFloat(input.val(), 10) == input.attr("min")) {
            ...
            if (parseFloat(input.val(), 10) == input.attr("max")) {
                $(this).attr("disabled", true);
            }
        }
    } else {
...
$(".input-number").on("change", function() {
    minValue = parseFloat($(this).attr("min"), 10);
    maxValue = parseFloat($(this).attr("max"), 10);
    valueCurrent = parseFloat($(this).val(), 10);
    old_href = $("#detail_id").data("url");
    href = $("#detail_id").prop(
        "href",
        `${old_href}` + "&quantity=" + `${valueCurrent}`
    );
    name = $(this).attr("name");
            ...
            for (var i = 0; i < filesLength; i++) {
                ...
                fileReader.onload = function(e) {
                    ...
                    $(
                        ...
                        file.name +
                        '"/>' +
                        '<br/><span class="removes"><i class="icon-cancel-music"></i></span>' +
                        "</span>"
                    ).insertAfter("#files");
