...
namespace App\Http\Requests\FreeShip;
...
class UpdateRequest extends FormRequest
{
    ...
    {
        return [
            'name' => 'required|unique:free_ships,name,' . $this->id,
            'slug' => 'required|unique:free_ships,slug,' . $this->id,
            'discount' => 'required|min:1',
            'start_date' => 'required|date',
            'end_date' => 'required|date|after:' . request()->start_date,
            'code' => 'required|min:6|unique:free_ships,code,' . $this->id,
