                        ...
                        <div class="card-body">
                                ...
                                <tbody>
                                    @foreach ($categoryBlog as $value)
                                        <tr id="{{ $value->id }}">
                                            <td>{{ $value->name }}</td>
                                            <td>{{ $value->slug }}</td>
                                            <td>{{ $value->getBlogs->count() ? $value->getBlogs->count() : 0 }}</td>
                                            @if ($value->status == 0)
                                                <td><a href="{{ route('admin.categoryBlog.unactive', $value->slug) }}"><span><i
                                                                class="fa fa-thumbs-o-down text-danger"
                                                                aria-hidden="true"></i></span></a></td>
                                            @else
                                                <td><a href="{{ route('admin.categoryBlog.active', $value->slug) }}"><span><i
                                                                class="fa fa-thumbs-o-up text-success"
                                                                aria-hidden="true"></i></span></a>
                                                </td>
                                            @endif
                                            <td>
                                                <a href="{{ route('danh-muc-bai-viet.edit', $value->slug) }}"
                                                    class="btn ">
                                                    <span><i class="fa fa-pencil-square-o text-primary"
                                                            aria-hidden="true"></i></span></a>
                                                <a href="{{ route('danh-muc-bai-viet.destroy', $value->slug) }}"
                            ...
                            <div class="col-sm-12 col-xs-12 d-flex">
                                <div class="col-sm-6 col-xs-12">
                                    <a href="{{ route('danh-muc-bai-viet.create') }}" class="btn btn-outline-success">Thêm mới
                                        danh mục</a>
                                </div>
                                <div class="col-sm-6 col-xs-12">
