                        ...
                        <div class="card-body">
                                ...
                                <tbody id="cate">
                                    @foreach ($category as $value)
                                        <tr id="{{ $value->id }}">
                                            <td>{{ $value->name }}</td>
                                            <td>{{ $value->slug }}</td>
                                            <td>{{ $value->getProduct->count() ? $value->getProduct->count() : 0 }}</td>
                                            @if ($value->status == 0)
                                                <td><a href="{{ route('admin.category.unactive', $value->slug) }}"><span><i
                                                                class="fa fa-thumbs-o-down text-danger"
                                                                aria-hidden="true"></i></span></a></td>
                                            @else
                                                <td><a href="{{ route('admin.category.active', $value->slug) }}"><span><i
                                                                class="fa fa-thumbs-o-up text-success"
                                                                aria-hidden="true"></i></span></a>
                                                </td>
                                            @endif
                                            <td>
                                                <a href="{{ route('danh-muc.edit', $value->slug) }}" class="btn">
                                                    <span><i class="fa fa-pencil-square-o text-primary"
                                                            aria-hidden="true"></i></span></a>
                                                <a class="deleteRecord"
                                                    href="{{ route('danh-muc.destroy', $value->slug) }}"
                            ...
                            <div class="col-sm-12 col-xs-12 d-flex">
                                <div class="col-sm-6 col-xs-12">
                                    <a href="{{ route('danh-muc.create') }}" class="btn btn-outline-success">Thêm mới
                                        danh mục</a>
                                </div>
                                <div class="col-sm-6 col-xs-12">
